...
def visualize_loss(df_metrics: pd, save_path: Path | None = None) -> None:
    ...
    ax.plot(
        df_metrics["epoch"], df_metrics["val_loss"], label="Validation Loss"
    )
...
def visualize_f1_precision_recall(
    df_metrics: pd, save_path: Path | None = None
) -> None:
    ...
    ax.plot(
        df_metrics["epoch"], df_metrics["train_recall"], label="Train Recall"
    )
    ...
    ax.plot(
        df_metrics["epoch"], df_metrics["val_recall"], label="Validation Recall"
    )
...
def visualize_accuracy(df_metrics: pd, save_path: Path | None = None) -> None:
...
def visualize_model_confidence(
    evaluation_report: pd.DataFrame, save_path: Path | None = None
) -> None:
    """
...
def compute_accuracy(evaluation_report: pd.DataFrame) -> float:
